...
private:
  ...
  // the port portAverage itself
  zmq_socket_ext portAverage;
...
public:
  ...
  MovingAverageModule(): ns_socket(NULL), cmd_socket(NULL) {
    ...
    portAverage.sock = new zmq::socket_t(*context, ZMQ_REQ);
    portAverage.ready = true;
    zmq_sockets.push_back(&portAverage);
  }
  
  ~MovingAverageModule() {
    delete portInput.sock;
    delete portAverage.sock;
  }
  ...
  void Init(std::string module_id) {
    ...
    
    {
      ...
      portName << "/movingaveragemodule" << module_id << "/input";
    ...
    }
    {
      ...
      std::stringstream portName; portName.str(); portName.clear();
      portName << "/movingaveragemodule" << module_id << "/average";
      portAverage.name = portName.str();
      std::string resolve = "/resolve" + portName.str();
      pns_record record;
      record.name = resolve;
      std::stringstream ss; ss.clear(); ss.str("");
...
protected:
  ...
  inline bool writeAverage(const int output) {
    ...
    bool state = portAverage.ready;
    SendRequest(portAverage.sock, state, false, ss.str());
    if (state) portAverage.ready = false;
    if (!portAverage.ready) {
      bool ack_state = true;
      ReceiveAck(portAverage.sock, ack_state, true);
      if (ack_state) {
        portAverage.ready = true;
        return true;
      }
