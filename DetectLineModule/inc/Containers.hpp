...
// The "dobots" namespace is not really necessary, but will at times prevent clashes with standard std functionality.
namespace dobots {
...
{
	// concept requirements
// error: ‘ForwardIteratorConcept’ is not a member of ‘__gnu_cxx’
//	__glibcxx_function_requires(ForwardIteratorConcept<ForwardIterator>);
// error: ‘LessThanComparableConcept’ is not a member of ‘__gnu_cxx’
...
T distance(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2, DistanceMetric metric) {
	...
	if (std::distance(first2,last2) != std::distance(first1,last1)) {
		std::cerr << "Container size: " << std::distance(first1,last1) << " != " << std::distance(first2,last2) << \
				std::endl;
 ...
 * For that reason combine_pairwise calls a given function for each of these unique pairs.
 *
